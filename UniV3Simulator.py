...
import random
import pandas as pd
from brownie.exceptions import ContractNotFound, VirtualMachineError
...
class UniV3Simulator:
    def __init__(
        ...
        supply_token1=1e18,
        fee_tier=3000,
        initial_pool_price=1,
        deployer=GOD_ACCOUNT,
        sync_pool_with_liq=False,
        initial_liquidity_amount=0,
    ):
        ...
        self.base_fee = latest_block.get("baseFeePerGas", 0)
    ...
    def deploy_load_tokens(self):
        ...
        def deploy_and_save_token(name, symbol, decimals, supply, key):
            token = SimpleToken.deploy(
                ...
                decimals,
                toBase18(supply),
                {"from": self.deployer, "gas_price": self.base_fee + 1},
            )
        ...
        if "token1_address" in pool_addresses:
            try:
                ...
                self.token1 = deploy_and_save_token(
                    self.token1_name,
                    self.token1_symbol,
        ...
        if "token0_address" in pool_addresses:
            try:
                ...
                self.token0 = deploy_and_save_token(
                    self.token0_name,
                    self.token0_symbol,
    ...
    def deploy_load_pool(self):
        ...
        if "pool_address" in pool_addresses:
            try:
                ...
                print(
                    f"Existing pool:{self.pool_id} having pool address: {self.pool} loaded"
                )
                self.align_tokens_with_pool()
                return
            except ContractNotFound:
                pass

        self.factory = UniswapV3Factory.deploy(
            {"from": self.deployer, "gas_price": self.base_fee + 1}
        )
        pool_creation_txn = self.factory.createPool(
            self.token0.address,
            self.token1.address,
            self.fee_tier,
            {"from": self.deployer, "gas_price": self.base_fee + 1},
        ...
        tx_receipt = self.pool.initialize(
            sqrtPriceX96, {"from": self.deployer, "gas_price": self.base_fee + 100000}
        )
    ...
    def align_tokens_with_pool(self):
        pool_token0 = self.pool.token0()
        pool_token1 = self.pool.token1()

        if (
            self.token0.address.lower() == pool_token0.lower()
            and self.token1.address.lower() == pool_token1.lower()
        ):
            return

        if (
            self.token1.address.lower() == pool_token0.lower()
            and self.token0.address.lower() == pool_token1.lower()
        ):
            self.token0, self.token1 = self.token1, self.token0
            self.token0_name, self.token1_name = self.token1_name, self.token0_name
            ...
            self.supply_token0, self.supply_token1 = (
                self.supply_token1,
                self.supply_token0,
            )
            return
        ...
        raise ValueError(
            f"Pool token order mismatch. pool.token0={pool_token0}, pool.token1={pool_token1}, "
            f"local.token0={self.token0.address}, local.token1={self.token1.address}"
        )
    ...
    def ensure_token_order(self):
        ...
        if int(self.token0.address, 16) > int(self.token1.address, 16):
            ...
            print(
                f"New {self.token0_symbol} token deployed at {self.token0.address} to ensure desired token order in the pool"
            )
    ...
    def sync_pool_state(self):
        ...
        if self.sync_pool_with_liq:
            ...
            for position in range(1, num_positions + 1):
                ...
                price_lower = self.initial_pool_price - (
                    price_increment * position * self.initial_pool_price
                )
                price_upper = self.initial_pool_price + (
                    price_increment * position * self.initial_pool_price
                )
                ...
                liquidity_amount = self.initial_liquidity_amount * random.uniform(
                    0.5, 1.5
                )
                ...
                # Add liquidity
                self.add_liquidity(
                    self.deployer, tick_lower, tick_upper, liquidity_amount, b""
                )
                print(
                    f"Position {position}: Added liquidity amount {liquidity_amount} in the price range {price_lower} to {price_upper}"
                )
    ...
    def add_liquidity(
        self, liquidity_provider, tick_lower, tick_upper, usd_budget, data
    ):
        tx_params = {
            "from": str(liquidity_provider),
            "gas_price": self.base_fee + 1,
            "gas_limit": 5000000,
            "allow_revert": True,
        }
        tx_params1 = {
            "from": str(GOD_ACCOUNT),
            "gas_price": self.base_fee + 1,
            "gas_limit": 5000000,
            "allow_revert": True,
        }
        ...
        try:
            ...
            tx_receipt = pool_actions.mint(
                liquidity_provider, tick_lower, tick_upper, liquidity, data, tx_params
            )
            ...
            print(tx_receipt.events)
            if amount0 > 0:
                tx_receipt_token0_transfer = self.token0.transfer(
                    self.pool.address, amount0, tx_params
                )
            if amount1 > 0:
                tx_receipt_token1_transfer = self.token1.transfer(
                    self.pool.address, amount1, tx_params
                )
        ...
        for position in all_positions[self.pool_id][liquidity_provider_str]:
            if (
                ...
                and position["tick_upper"] == tick_upper
            ):
        ...
        if existing_position:
            ...
            existing_position[
                "amount_usd"
            ] += usd_budget  # Add new liquidity to existing position
        else:
    ...
    def add_single_sided_liquidity(
        self, liquidity_provider, tick_lower, tick_upper, usd_budget, data
    ):
        tx_params = {
            "from": str(liquidity_provider),
        ...
        try:
            pool_actions = self.pool
            liquidity = self.budget_to_liquidity_single_sided(
                tick_lower, tick_upper, usd_budget
            )
            # print(liquidity)
            ...
            if amount0 > 0:
                ...
                print(
                    f"token1 amount:{amount0}transfered to contract:{tx_receipt_token0_transfer}"
                )
            if amount1 > 0:
                tx_receipt_token1_transfer = self.token1.transfer(
                    self.pool.address, amount1, tx_params
                ...
                print(
                    f"token1 amount:{amount1}transfered to contract:{tx_receipt_token1_transfer}"
                )
    ...
    def add_liquidity_with_liquidity(
        self, liquidity_provider, tick_lower, tick_upper, liquidity, data
    ):
        tx_params = {
            "from": str(liquidity_provider),
            "gas_price": self.base_fee + 1,
        ...
        if existing_position:
            ...
            existing_position[
                "amount_usd"
            ] += liquidity  # Add new liquidity to existing position
        else:
            # Add new position to list
            all_positions[self.pool_id][liquidity_provider_str].append(
                {
    ...
    def remove_liquidity(self, liquidity_provider, tick_lower, tick_upper, amount_usd):
        ...
        if (
            self.pool_id not in all_positions
            or liquidity_provider_str not in all_positions[self.pool_id]
        ):
        ...
        if existing_position["liquidity"] > liquidity:
            ...
            all_positions[self.pool_id][liquidity_provider_str].remove(
                existing_position
            )  # Remove position if liquidity becomes zero
    ...
    def remove_liquidity_with_liquidty(
        self, liquidity_provider, tick_lower, tick_upper, liquidity, collect_tokens=True
    ):
        liquidity_provider_str = str(liquidity_provider)
        tx_receipt = None
    ...
    def swap_token0_for_token1(self, recipient, amount_specified, data):
        tx_params = {
            "from": str(recipient),
            "gas_price": self.base_fee + 1000000,
            "gas_limit": 5000000,
            "allow_revert": True,
        }
    ...
    def collect_fee(self, recipient, tick_lower, tick_upper, poke=False):
        ...
        position_key = Web3.solidityKeccak(
            ["address", "int24", "int24"], [str(recipient), tick_lower, tick_upper]
        ).hex()
        ...
        fee_collected_usd = 0
        try:
            tx_receipt = self.pool.collect(
                recipient, tick_lower, tick_upper, amount0Owed, amount1Owed, tx_params
            )
            print(tx_receipt.events)
            ...
            slot0_data = self.pool.slot0()
            fee_collected_usd = fromBase18(
                amount0Collected * sqrtp_to_price(slot0_data[0]) + amount1Collected
            )
        except VirtualMachineError as e:
            print("Fee collection failed:", e.revert_msg)
            print(
                f"contract_token0_balance - amount0Owed: {self.token0.balanceOf(self.pool)-amount0Owed} ,contract_token1_balance - amount1Owed: {self.token1.balanceOf(self.pool)-amount1Owed}, position_tick_lower: {tick_lower}, position_tick_upper: {tick_upper}"
            )
    ...
    def get_lp_all_positions(self, liquidity_provider):
        ...
        if liquidity_provider_str in all_positions:
            ...
            print(
                f"No positions found for this liquidity provider {liquidity_provider_str} in pool."
            )
            return None

    def get_position_state(self, tick_lower, tick_upper, agent):
        position_key = Web3.solidityKeccak(
            ["address", "int24", "int24"], [str(agent), tick_lower, tick_upper]
        ).hex()
    ...
    def get_pool_state_for_all_ticks(
        self, lower_price_interested, upper_price_interested
    ):
        tick_states = {}
        try:
            with open("model_storage/liq_positions.json", "r") as f:
        ...
        lower_tick_interested = price_to_valid_tick(
            lower_price_interested, tick_spacing=60
        )
        upper_tick_interested = price_to_valid_tick(
            upper_price_interested, tick_spacing=60
        )
        ...
        for liquidity_provider, positions in all_positions[self.pool_id].items():
            for position in positions:
                ...
                if (
                    lower_tick_interested <= tick_lower <= upper_tick_interested
                    or lower_tick_interested <= tick_upper <= upper_tick_interested
                ):
        ...
        for tick in unique_ticks:
            tick_states[tick] = self.get_tick_state(
                tick
            )  # Fetch and store each tick state
    ...
    def get_pool_state_for_all_positions(self):
        ...
        for liquidity_provider_str, positions in all_positions[self.pool_id].items():
            for position in positions:
                ...
                position_states[position_key] = self.get_position_state(
                    tick_lower, tick_upper, liquidity_provider_str
                )
    ...
    def budget_to_liquidity(self, tick_lower, tick_upper, usd_budget):
        ...
        if sqrtp_cur <= sqrtp_low:  # Current price is below the range
            ...
            x_to_y_ratio = calculate_x_to_y_ratio(
                P=sqrtp_to_price(sqrtp_cur),
        ...
        liquidity = get_liquidity_for_amounts(
            sqrt_ratio_x96=sqrtp_cur,
            sqrt_ratio_a_x96=sqrtp_low,
            sqrt_ratio_b_x96=sqrtp_upp,
            amount0=amount_token0,
            amount1=amount_token1,
        )
